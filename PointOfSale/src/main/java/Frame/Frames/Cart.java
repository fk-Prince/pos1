...
import Repository.PaymentRepository.PaymentServices;
import Repository.PaymentRepository.IPayment;
...
public class Cart extends JPanel {
    ...
    private final IPayment iBuyProduct;

    public Cart(Queue<Payment> cartProduct, TableRefresh refreshTable) {
        iBuyProduct = new PaymentServices();
        ...
        if (!cartProduct.isEmpty()) {
            ...
            button.addActionListener(e -> {
                ...
                for (Payment pay : cartProduct) {
                    success = iBuyProduct.doPayment(pay);
                }
                if (success) {
                    cartFrame.dispose();
    ...
    private double calculateTotal() {
        return cartProduct.stream().mapToDouble(Payment::compute).sum();
    }
}
