...
public class ItemListTable extends JPanel implements TableRefresh {
    ...
    private ArrayList<ProductEntity> listOfItems;
    ...
    @Override
    public boolean isPaymentValid(ProductEntity product) {
        for (ProductEntity p : listOfItems) {
            if (product.getProductId() == p.getProductId()) {
                ...
                if (stock > 0) {
                    listOfItems.add(new ProductEntity(p.getProductId(), p.getProductBrandName(), p.getProductPrice(), stock));
                }
                break;
            }
        }
    ...
    public void refreshTable() {
        model.setRowCount(0);
        for (ProductEntity product : listOfItems) {
